"""Reshape-based Conv1d->Conv2d with explicit shape values from inference."""
import onnx
...
# First, infer shapes on the ORIGINAL model to know all intermediate dims
...
print('Shape map has {} entries'.format(len(shape_map)))
...
for node in nodes:
    if node.op_type in ('Conv', 'ConvTranspose') and len(node.input) >= 2:
        ...
        if w is not None and len(w.shape) == 3:
            ...
            # Get input shape from shape_map
            ...
            # Get output shape from shape_map
...
out_path = '/project/aimet_work/generator_cle_2d_reshape_v2.onnx'
onnx.save(m, out_path)
print('Converted {} ops, saved {} ({:.1f}MB)'.format(converted, out_path, os.path.getsize(out_path)/1024/1024))
