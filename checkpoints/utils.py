...
def load_checkpoint(checkpoint_path, model, optimizer=None, skip_optimizer=False):
    ...
    for k, v in state_dict.items():
        saved_value = saved_state_dict.get(k)
        if saved_value is None:
            if "ja_bert_proj" in k:
                v = torch.zeros_like(v)
                logger.warning(
                    f"Using zero init for missing {k} from an older checkpoint"
                )
            else:
                logger.warning(
                    f"Checkpoint is missing {k}; keeping model initialization with shape {tuple(v.shape)}"
                )
            new_state_dict[k] = v
            continue

        if saved_value.shape != v.shape:
            logger.warning(
                f"Checkpoint shape mismatch for {k}: checkpoint {tuple(saved_value.shape)} != model {tuple(v.shape)}; keeping model initialization"
            )
            new_state_dict[k] = v
            continue

        new_state_dict[k] = saved_value
